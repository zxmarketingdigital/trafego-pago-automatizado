#!/usr/bin/env python3
"""Instalador opt-in do Bônus 3 — relatório diário no WhatsApp."""
...
PLIST_NAME = "com.zxlab.meta-report-whatsapp.plist"
PLIST_SRC = REPO_ROOT / "launchagents" / PLIST_NAME
PLIST_DST = Path.home() / "Library" / "LaunchAgents" / PLIST_NAME
SCRIPT_NAMES = ("report_formatter.py", "send_whatsapp_report.py")
...
def _install_plist() -> bool:
    ...
    try:
        ...
        # unload antes do load torna a reinstalação segura e atualiza o job existente.
...
def main() -> int:
    ...
    print("\n🔎 Validando configuração e mostrando a mensagem sem enviar…")
    dry_run = subprocess.run(
        [sys.executable, str(SCRIPT_DST_DIR / "send_whatsapp_report.py"), "--dry-run"],
        check=False,
    )
    ...
    try:
        answer = input(
            "\nA mensagem acima está correta? Instalar envio diário às 8h20? [s/N]: "
        ).strip().lower()
    ...
    if not _install_plist():
        return 1
    print("✅ Relatório no WhatsApp agendado diariamente às 8h20.")
    return 0
